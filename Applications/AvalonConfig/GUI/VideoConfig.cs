...
{
    public class VideoConfig : GUIWindow
    {
        #region Enums
       ...
       protected GUIButtonControl btnThumbViewMod = null;
        ...
        private void ApplyConfigurationChanges()
        {
            ...
            //share view
            string skinFile = GUIGraphicsContext.Skin + @"\myvideo.xml";

            string style = btnStyle.Selected ? "default" : "noinfo";

            // Set <import> paths

            AvalonHelper.SetSkinImport(skinFile, "Background", string.Format("myvideo.background.{0}.xml", style));
            AvalonHelper.SetSkinImport(skinFile, "Views", string.Format("myvideo.views.{0}.xml", style));
            AvalonHelper.SetSkinImport(skinFile, "Facade", string.Format("common.facade.video.{0}.xml", style));
            //Log.Info(skinFile);
            ...
            switch (ThumbViewMod)
            {
                ...
                case Views.ThumbsNoFanart:
                    AvalonHelper.SetSkinImport(skinFile, "Background", string.Format("myvideo.background.{0}.thumbs.nofanart.xml", style));
                    AvalonHelper.SetSkinImport(skinFile, "Facade", string.Format("common.facade.video.{0}.thumbs.nofanart.xml", style));
                    break;

                case Views.ThumbsXL:
                    AvalonHelper.SetSkinImport(skinFile, "Background", string.Format("myvideo.background.{0}.thumbs.XL.xml", style));
                    AvalonHelper.SetSkinImport(skinFile, "Facade", string.Format("common.facade.video.{0}.thumbs.XL.xml", style));
                    break;
            }

            //database view
            skinFile = GUIGraphicsContext.Skin + @"\myvideoTitle.xml";
        ...
        public override int GetID
        {
            get
            {
                return (int)AvalonGUIConfig.AvalonScreenID.AvalonVideoConfig;
            }
        }

        public override bool Init()
        {
            return Load(GUIGraphicsContext.Skin + @"\AvalonConfig_video.xml");
        }
